...
class TanhLayer(tf.keras.layers.Dense):
    def call(self, x):
        strength = super().call(x)
        activation = tf.nn.tanh(strength)
        return activation, strength
...
class MLP(tf.keras.Model):
    def __init__(
        ...
        print(f"Constructing layers with name {layer_constructor.__name__}")
        print(f"Hideen sizes are {n_hiddens}")

        self.hidden_layers = [
            layer_constructor(n_hidden, kernel_regularizer=self.regularizer)
            for n_hidden in n_hiddens
        ]
        self.final_layer = layer_constructor(1, kernel_regularizer=self.regularizer)
